...
public class LoopEndCommandTest {
    ...
    @Test
    public void adviceOnFlow_byteIs0_advicesToGetNextInstruction() throws Exception {
        val command = new LoopEndCommand();
    ...
    @Test
    public void adviceOnFlow_byteIsNot0_advicesToGoBackToLoopStart() throws Exception {
        val command = new LoopEndCommand();
        val memoryFake = Mockito.mock(Memory.class);
        Mockito.when(memoryFake.get()).thenReturn((byte) 4);
        val result = command.adviceOnFlow(memoryFake);

        Assertions.assertThat(result).isEqualTo(BrainFuckFlowAdvice.JUMP_TO_LOOP_START);
